...
独立 SDK 开发仓：微内核（`core/`）、开发者套件（`sdk/`）、工作台底座（`workbench/`）、最小可跑模板（`templates/local-app/`）。事实顺序：源码与针对性测试 > `DOCUMENTS/mental-model.md` > 其他文档。

## 目录

- `core/` — `@graphvideo/kernel`：零业务语义微内核（mailbox/change 调度、State、Projection）。
...
npm test                 # core + unit + ui
npm run build:sdk        # 构建四个包（core/backend-sdk/sdk/workbench）
npm run export:sdk       # 构建 + 组装 dist/delivery + 仓外三验收
```

## 红线（摘要）

- `core/src` 零业务，不得导入业务代码；`workbench/` 不得导入具体业务插件。
