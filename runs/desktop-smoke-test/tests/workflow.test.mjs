...
describe('desktop-smoke-test workflow', () => {
  ...
  it('browser and computer action plus observation reach world review without a document edit', async () => {
    ...
    runtime.inject({ targetNodeId: 'smoke/entry', info: {
    ...
    expect(runtime.getState('smoke/session')).toMatchObject({
      status: 'awaiting-world-save', desktopActionResult: { command: 'focus_window', focused: true },
    })
    expect(runtime.getState('smoke/world-review').pendingConfirmation).toMatchObject({ step: 'save-world' })
  ...
  it('TriggerSmokeTest 在浏览器检查后停住等待确认，批准后才跑通并结算 done', async () => {
    ...
    expect(runtime.getState('smoke/doc-edit-gate').pendingConfirmation).toMatchObject({
      step: 'edit-doc', requestId: 'smoke-1', docName: '开发步骤.docx',
    })
    expect(calls.some((call) => call.adapter === 'desktopControl')).toBe(false)

    runtime.inject({
      targetNodeId: 'smoke/doc-edit-gate',
    ...
    expect(runtime.getState('smoke/world-review').pendingConfirmation).toMatchObject({ step: 'save-world', requestId: 'smoke-1' })
    runtime.inject({ targetNodeId: 'smoke/world-review', info: {
    ...
    expect(runtime.getState('smoke/world-review').pendingConfirmation).toBeNull()
  ...
  it('decline at world save ends without writing a world document', async () => {
    const { nodes, calls } = assemble()
    const runtime = createTestRuntime({ nodes })
    runtime.inject({ targetNodeId: 'smoke/entry', info: { type: 'TriggerSmokeTest', requestId: 'decline' } })
    await runtime.waitForQuiescence()
    runtime.inject({ targetNodeId: 'smoke/doc-edit-gate', info: {
  ...
  it('surfaces agent review failure instead of leaving the save breakpoint pending', async () => {
    ...
    try {
      ...
      await runtime.waitForQuiescence()
      expect(runtime.getState('smoke/session')).toMatchObject({
        status: 'error', lastError: 'model API key is required',
      })
      expect(runtime.getState('smoke/world-review').pendingConfirmation).toBeNull()
      expect(calls.some((call) => call.adapter === 'worldDocument')).toBe(false)
    } finally { runtime.dispose() }
  ...
  it('拒绝关键步骤时会话进入 error 并记录原因', async () => {
    ...
    expect(runtime.getState('smoke/doc-edit-gate').pendingConfirmation).toBeNull()
  ...
  it('忽略旧请求的确认与迟到结果', async () => {
    const { nodes, calls } = assemble()
    const runtime = createTestRuntime({ nodes })
    runtime.inject({ targetNodeId: 'smoke/entry', info: { type: 'TriggerSmokeTest', requestId: 'current' } })
    await runtime.waitForQuiescence()
    runtime.inject({ targetNodeId: 'smoke/doc-edit-gate', info: { type: 'ConfirmStepInfo', requestId: 'old', step: 'edit-doc', decision: 'approve' } })
