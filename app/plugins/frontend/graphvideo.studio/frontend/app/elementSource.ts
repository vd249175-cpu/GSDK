...
import { elementModules as pluginElementModules } from 'virtual:graphvideo-elements'
...
function toCandidate(descriptor: SourceElementDescriptor): ElementCandidate {
  return {
    ...
    async load() {
      const key = `${descriptor.pluginId}/${descriptor.elementId}`
...
export class DesktopElementSource {
  ...
  private async apply(catalog: ElementSourceCatalog) {
    ...
    const preferredWorkspaceId = loadActiveWorkspacePreference() ?? applicationDefinition.defaultWorkspace
