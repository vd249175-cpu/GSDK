...
export interface OsRecorderBridge {
...
const getBridge = (): OsRecorderBridge | undefined =>
  (window as unknown as { recorder?: OsRecorderBridge }).recorder
...
export function App() {
  ...
  const start = async () => {
    ...
    try {
      ...
      setState(await bridge.start(sessionId))
    } finally {
      setBusy(false)
    }
  }
  ...
  const copyArtifact = async () => {
    const bridge = getBridge()
    if (!bridge || !state.artifactPath) return
    await bridge.copyToClipboard(state.artifactPath)
    setCopied(true)
      ...
      <header className="recorder-topbar">
        ...
        <div className="topbar-actions">
          ...
          {state.artifactPath && !isRecording && (
            <>
              <button type="button" className="action-btn" onClick={() => getBridge()?.openArtifact()}>
                <FolderOpen size={13} />
                <span>打开产物</span>
              </button>
