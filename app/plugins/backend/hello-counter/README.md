...
本插件 ID 为 `example.hello-counter`，创建 `example.counter`。唯一公开用户意图为 `IncrementInfo`，在该 Owner 的单飞 change 中增加 count；读取通过 Projection/EncodedValue，不另造读取 Info。默认应用（`graphframework-demo`）没有启用此插件，它用于最小后端测试和离线诊断，不贡献 Element 或 Workspace。
...
node app/plugins/backend/hello-counter/scripts/diagnose.mjs path state:example.counter::count change:example.counter::IncrementInfo
...
`scripts/diagnose.mjs` 使用 `analysis/links.mjs` 显式声明的 counter 入口与投影关系，读取已构造实例，不创建生产 Runtime。`analysis/config.json` 是消费方配置，不会由内核自动读取。诊断结果是静态关系证据，不代表正在运行的 run 状态。运行中应用的观测与分析入口见[调试指南](../../../../DOCUMENTS/diagnostics/debug-guide.md)。
