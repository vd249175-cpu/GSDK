...
type: package
title: OS Recorder
description: OS 级操作录制会话插件（UFO 侧车经 EffectAdapter 接入）
...
唯一职责：维护 OS 级操作录制会话 State（开始/停止/事件计数），执行与观察
物理分离，UFO 本体永远在进程外侧车。

明确不承担：UFO 部署与模型推理、真实 OS hook 实现、录制自动化策略
（策略归未来的独立 `recording-automation` 插件）、前端（本插件 backend-only，
无用户操作面之外的 Element/Workspace）。

## 公开契约

- 目标 Node：`<instance>/session`（Owner，心智：`example.os-recorder/session`）。
- 可接收 Info：
  ...
  - `RecordingStatusInfo { status, sessionId, handle? }`（仅 execution 发送）；
  - `RecordingEventInfo { sessionId, event }`（仅 observation 发送）。
- 公开 Projection：`{ status, sessionId, handle, eventCount, lastEvent, lastError }`。
...
- `RecordingCaptureNode`（execution）：只调 `ufo/capture-control` 下发 start/stop，不监听。
- `RecordingObserverNode`（observation）：只调 `ufo/capture-events` 轮询并转发，不下发动作。
