import { parseProject } from '../markdown/markdown-parser';
import {
  type NodeType,
  type NodeSymbol,
  type ParsedProject,
...
export type { ProjectTreeItem, ProjectIssue };
...
function structureBounds(lines: string[]) {
  ...
  const closing = lines.findIndex(
    (line, index) => index > opening && line.trim() === structureCloseTag,
  );
...
function assertUniqueNodeTitle(
  parsed: ParsedProject,
  title: string,
...
function renameItem(
  ...
  if (item.kind === 'node') {
    ...
    flattenProjectTree(parsed.tree)
      .filter(
        (entry) =>
          (item.nodeId && entry.nodeId === item.nodeId) || entry.title === item.title,
      )
      .forEach((entry) => {
...
function reindentBlock(lines: string[], sourceIndent: number, targetIndent: number) {
  ...
  return lines.map((line) => {
    ...
    return `${targetPrefix}${
      line.startsWith(sourcePrefix) ? line.slice(sourceIndent) : line.trimStart()
    }`;
  });
}
...
function moveItem(
  ...
  const adjustedInsertion =
    insertion > sourceBlock.start ? insertion - sourceLines.length : insertion;
...
function adjustDepth(
  ...
  if (operation.direction === 'right') {
    ...
    if (operation.includeChildren) {
      ...
      document.lines[source.line - 1] = `  ${indentation}${document.lines[
        source.line - 1
