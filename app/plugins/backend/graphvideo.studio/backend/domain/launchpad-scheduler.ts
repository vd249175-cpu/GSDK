import type { NodeType, ProjectTreeItem } from './types';
...
export class LaunchpadScheduler {
  ...
  public static buildLaunchpadItems(
    ...
    const resolveItemNodeId = (item: ProjectTreeItem): string | undefined => {
    ...
    if (tree && Array.isArray(tree)) {
      const walkTree = (items: ProjectTreeItem[], parentMediaId?: string) => {
        for (const item of items) {
          const realNodeId = resolveItemNodeId(item);

          // 收集当前子树中所有的媒体子节点真实 ID (图像/视频/音频)
          const collectMediaDescendants = (children: ProjectTreeItem[]): string[] => {
            const result: string[] = [];
