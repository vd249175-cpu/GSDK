...
export class LaunchpadScheduler {
  ...
  public static buildLaunchpadItems(
    ...
    tree?: import('../../frontend/core/project/types').ProjectTreeItem[],
    ...
    const resolveItemNodeId = (item: import('../../frontend/core/project/types').ProjectTreeItem): string | undefined => {
    ...
    if (tree && Array.isArray(tree)) {
      const walkTree = (items: import('../../frontend/core/project/types').ProjectTreeItem[], parentMediaId?: string) => {
        for (const item of items) {
          const realNodeId = resolveItemNodeId(item);

          // 收集当前子树中所有的媒体子节点真实 ID (图像/视频/音频)
          const collectMediaDescendants = (children: import('../../frontend/core/project/types').ProjectTreeItem[]): string[] => {
            const result: string[] = [];
