interface Window {
  graphvideoDesktop?: {
    ...
    graphKernel?: {
      request<K extends keyof import('@graphvideo/sdk/protocol').GraphSyscallRequestMap>(
        method: K,
        input: import('@graphvideo/sdk/protocol').GraphSyscallRequestMap[K]['input'],
      ): Promise<import('@graphvideo/sdk/protocol').GraphSyscallRequestMap[K]['output']>
      subscribe<K extends keyof import('@graphvideo/sdk/protocol').GraphSyscallEventMap>(
        event: K,
        listener: (
          payload: import('@graphvideo/sdk/protocol').GraphSyscallEventMap[K]
        ) => void,
      ): () => void
    }
    project: {
    ...
    generationModels: {
      list(): Promise<{
        models: import('../backend/shared/generation-model.mjs').GenerationModelManifest[]
        issues: string[]
      }>
      ...
      prepareBatch(
        ...
        batchId: string
        info: Pick<import('../backend/protocol/generation-model-resolution').GenerationBatchRequestedInfo, 'type' | 'batchId'>
      }>
      ...
      import(): Promise<{
        canceled: boolean
        error?: string
        model?: import('../backend/shared/generation-model.mjs').GenerationModelManifest
      }>
