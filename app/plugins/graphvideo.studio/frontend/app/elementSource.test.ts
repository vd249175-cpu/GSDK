...
describe('DesktopElementSource plugin lifecycle', () => {
  ...
  it('loads real studio plugin from disk and registers all panels and workspaces', async () => {
    const { ElementCatalog } = await import('@graphvideo/desktop/element-catalog')
    const { resolve } = await import('node:path')
    const pluginsRoot = resolve(__dirname, '../../..')
    const catalogStore = new ElementCatalog({ pluginDirectories: { 'graphvideo.studio': resolve(pluginsRoot, 'graphvideo.studio') } })
