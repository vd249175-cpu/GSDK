...
当节点接收到关闭指令时：
- `StopInfo` 绝非系统特殊句法，只是 Node 在其自由表达的 `change` 代码中包含的普通判定逻辑（例如识别到关闭指令后标记自身，并在后续逻辑中直接拒绝处理其他业务 change）；
...
阶段 2: 间隙暴力置换 (Brute-force Swap & Discard)
  Space.evict(nodeId) ──────────────► [ 旧 Node 移除，其残留 Mailbox 与未达消息全部直接暴力丢弃！]
                                          │
  Space.admit(新 Node 实例, prevState) ► [ 新 Node 注入，继承上个状态版本 ]

阶段 3: 激活 (Resume)
  外部系统 ─── inject(StartInfo) ────► [ 新 Node 实例 ]
                                          │
                                       新代码以全新逻辑开始消费后续新到的业务 Info
```
...
| **M3: 生命周期平权与 Info 标准化** | 制定标准生命周期 Info 契约，消除特权生命周期方法。 | `sdk/contract/lifecycle.ts` | 编写示例节点，仅通过响应 `StopRequestedInfo` 完成自清洁与排空。 |
| **M4: 节点级热更新 API 与验证** | 交付 `space.replace(newNode)` 一键热替换方案与状态延续机制。 | `core/` 导出原子热更方法并在 `apps/` 中演示 | 编写热更集成测试：无需重启应用进程，动态替换 CounterNode 增量算法，验证状态保持且新算法即时生效。 |
...
2. **生命周期 Info 化与平权（启动/关闭皆普通消息）**；
3. **准入（Admit）与驱逐（Evict）的纯粹热更机制（无魔术缓冲，未达消息自然丢弃）**；
