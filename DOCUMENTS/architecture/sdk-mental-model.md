...
description: 面向平台和高级插件维护者的 SDK 包职责、依赖方向、分包层次与契约边界。
status: stable
...
系统从底层依赖到顶层业务遵循严格的三层分包与复用范式：

1. **直接第三方标准包（Direct Packages）**：
   - 官方在 npm / PyPI / crates.io 上有稳定发包的标准依赖（如 `playwright`、`numpy`、`vitest` 等）；
   - 源码不入仓，由各包目录的 `package.json` / `Cargo.toml` 显式声明，使用锁文件锁定版本。
2. **二次开发底层能力包（`packages/<name>`）**：
   - 官方无成熟发包、需二次开发或深度魔改的外部项目（如微软 Windows GUI 智能体项目 [UFO](https://github.com/microsoft/ufo)）；
   - 统一收敛至 `packages/<name>`（如 `packages/ufo/`）；
   - **纯能力库定位**：零 GraphFramework 业务语义（不导入 Node/Info/State），只暴露干净的 API/类/RPC 接口；
   - **消费通道**：仅由业务插件中 WorldNode 构造注入的 `EffectAdapter` 导入和调用，作为驱动物理世界的底层能力，保持独立演进与单测。
3. **应用层复用机制：节点装配复用，不做包处理**：
   - 在业务与应用层，当出现重复的业务实现或多工作流共享逻辑时，**坚决不做包处理（不新建 npm/能力包）**，避免包碎片化与依赖地狱；
   - **非核心插件不进入 `app/`**：`app/` 与 `app/plugins/` 严格仅承载核心底座与核心插件。业务工作流中的非核心插件绝不进入 `app/`，只在 run 下的特定工作流中（开发阶段在 `runs/<name>/plugins/`）；
   - **工作流插件并入机制**：特定工作流通过测试跑通后，可通过 `runs/main/plugins/` 并入主 run 工作流，但依然绝不进入核心 `app/`；
   - **核心复用手段是 `assembly.mjs` 节点装配**：多个 run 或工作流通过 `run.node({ id, plugin, factory })` 引用相同 Node 工厂；
   - **运行时按 ID 幂等去重**：装配器在运行前按 ID 自动去重相同实例，或通过 `requireNode` 声明依赖既有 Owner；业务协作统一依靠定向 `ctx.send`。
...
底层二次开发能力（如 UFO/定制智能体） → packages/<name>（纯能力库，由 Adapter 导入）
应用层重复业务逻辑复用 → runs/<name>/assembly.mjs（节点装配复用，按 ID 去重，不做包处理）
