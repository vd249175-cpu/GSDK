...
面向不使用 Git 的团队成员，通用知识与工作流说明共同维护在自建服务端；可执行的后端/前端与 `assembly.mjs` 组成版本化 ZIP，通过协作通道分享。基础软件与核心组件仍走批量更新，不复制进工作流能力包。完整发布、接收、摘要校验、自动去重和冲突处理顺序见[SDK、插件与协作分发契约](distribution-contract.md) §3。
