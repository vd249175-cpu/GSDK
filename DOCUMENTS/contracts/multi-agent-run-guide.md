...
description: 命名 run 隔离、目录所有权、Bash 启停生命周期和多 Agent 并行开发规范。
status: stable
tags: [multi-agent, run-environment, supervisor, isolation]
---
...
面向不使用 Git 的团队成员，通用知识与工作流正文共同维护在飞书知识库，成员自己的 `AGENTS.md` 只保存所需页面索引；可执行的后端/前端与 `assembly.mjs` 组成版本化 ZIP，通过指定飞书群聊分享。基础软件与核心组件仍走批量更新，不复制进工作流能力包。完整发布、接收、摘要校验、自动去重和冲突处理顺序见[SDK、插件与飞书协作分发契约](distribution-contract.md) §3。
