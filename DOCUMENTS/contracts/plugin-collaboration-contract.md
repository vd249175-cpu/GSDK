...
status: stable
tags: [plugins, collaboration, publishing, contract]
---
...
- 直接读取或修改其它插件的 State，或者用不同实现争用其它插件的 Node ID；多个工作流重复声明同一不可变 Node 组件属于正常复用，由 run 装配自动去重；
...
相关运行规则见 [Plugin SDK 当前边界](../guides/plugin-sdk-guide.md)、[当前心智模型](../architecture/mental-model.md)、[实例因果分析](../diagnostics/causal-analysis.md)和[Workbench 设计系统](../guides/design-system.md)。
