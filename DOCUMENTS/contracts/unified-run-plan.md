...
3. 启动物理进程前完成静态可完成的身份、模块、实例声明和切口校验；相同 ID、相同定义的插件、实例与前端声明先自动去重，不同定义的同 ID 声明报告冲突并由 Agent 处理；构造后验证实际 ID 唯一性及事实，再 admit。
...
**先写验证：**任意名称、其他 cwd、空格路径、外部插件目录、版本错误、缺失入口、相同声明自动去重、同 ID 不同定义冲突和无前端/无场景配置；并行构建两个 run，检查源码与缓存无交叉改写。
