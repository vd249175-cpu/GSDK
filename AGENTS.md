...
- **解说声音与字幕**：完成后的会话目录另含 `audio/narration.webm`、`narration-transcript.md` 与 `subtitles.srt`。对照操作与解说时，分别读取 `agent-transcript.md` 和 `narration-transcript.md`；`unified-events.json` 的 `narration` 字段关联两个轨道。全局原始声音与可修正字幕索引保存在 `recordings/narration/` 的 `<sessionId>.webm`、`subtitles.json` 和 `subtitles.srt`。
