#!/usr/bin/env node
/**
 * graphframework-init: 冷启动命令，用本包自带的 init-bundle 创建最小本地应用。
 * 只依赖 node 内置模块；模板、文档、安装包均来自 init-bundle/，除 react 系
 * 常规依赖外无需 registry 上的 @graphframework/*（bundle 内 tarball 本地满足，
 * 含 peer 的 kernel 同事务装入，不触发外网解析）。
 *
 * 用法：
 *   graphframework-init <目标目录> [--name <包名>] [--skip-install] [--help]
 *
...
const usage = [
  '用法：graphframework-init <目标目录> [--name <包名>] [--skip-install]',
  '  <目标目录>  不存在 / 为空 / 仅含脚手架三件套均可',
  '  --name      写入目标 package.json 的包名（默认 hello-graphframework）',
  '  --skip-install  跳过依赖安装（目标已装好四包时用）',
  '示例：',
  '  mkdir my-app && cd my-app && npm init -y',
  '  npm install /path/to/delivery/packages/*.tgz',
  '  npx graphframework-init . --skip-install && npm install && npm run verify',
].join('\n')
...
const appName = nameValue ?? 'hello-graphframework'
...
const missing = [
  ...
  !tarballs.some((f) => /graphframework-sdk-.+\.tgz$/.test(f)) && 'tarballs/graphframework-sdk-*.tgz',
].filter(Boolean)
if (missing.length > 0) {
  console.error(`[init] 此 @graphframework/sdk 包不含完整 init-bundle（缺：${missing.join('、')}），请用仓库 npm run export:sdk 重新打包`)
  process.exit(1)
}
...
// 4. 装包：四个 tarball 一次事务，实体目录安装、单副本，不碰外网 @graphframework/*。
