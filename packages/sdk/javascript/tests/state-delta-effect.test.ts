...
import { KernelRuntime } from '../src/node/runtime';
import { Node, WorldNode } from '../src/node/node';
import { MemoryTraceStore } from '../src/node/observation';
import type { DomainChangeContext, Info, WorldChangeContext, ChangeRecord } from '../src/protocol/types';
import type { EffectAdapter } from '../src/effect/effects';
...
class EffectNode extends WorldNode<{
    ...
    override async change(info: Info, ctx: WorldChangeContext<{
        ...
        await ctx.effectAdapter(this.adapter, { fail: info.fail as boolean | undefined });
    }
}
class LargeValueNode extends Node<{
    ...
    override change(info: Info, ctx: DomainChangeContext<{
        ...
        ctx?.write('value', info.value as string);
    }
}
