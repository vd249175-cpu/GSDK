...
import { ExecutionWorldNode, Node, ObservationWorldNode, WorldNode } from '../src/node/node';
    ...
    protected override async change(info: Info, ctx: DomainChangeContext<TestState>) {
        if (info.type === 'IncrementInfo') {
            await ctx.span('increment_step', () => {
                ...
                ctx.write('count', current + ((info.amount as number | undefined) ?? 1));
        ...
        if (info.type === 'AsyncStep') {
            const step = info.step as number;
