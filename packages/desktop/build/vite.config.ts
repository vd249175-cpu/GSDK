...
const virtualIds = ['virtual:graphvideo-application', 'virtual:graphvideo-elements', 'virtual:graphvideo-config']
export default defineConfig({
  root: resolve(runtimeRoot, 'renderer'), base: './',
  resolve: {
    dedupe: ['react', 'react-dom', 'lucide-react'],
    alias: [
      { find: '@graphvideo/sdk/protocol', replacement: resolve(runtimeRoot, '../sdk/javascript/src/protocol/index.ts') },
      ...
      { find: /^@graphvideo\/theme$/, replacement: resolve(runtimeRoot, '../frontend/theme/index.css') },
  ...
  plugins: [react(), {
    ...
    load(id) {
      ...
      if (id === '\0virtual:graphvideo-application') return `import ${JSON.stringify(normalizePath(application.rendererEntry))};`
      if (id === '\0virtual:graphvideo-elements') {
    ...
    transformIndexHtml(html) {
      return html.replace('<title>GraphVideo Studio</title>', `<title>${String(application.definition.name).replace(/[<>&]/g, '')}</title>`)
    },
  }],
