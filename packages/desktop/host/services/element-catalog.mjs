...
export class ElementCatalog {
  constructor({ elementsRoot, workspacesRoot, pluginsRoot, pluginDirectories = null, pluginIds = null }) {
  ...
  async scan() {
    ...
    if (this.pluginsRoot || this.pluginDirectories) {
      const directories = this.pluginDirectories
        ? Object.entries(this.pluginDirectories)
        : (await directoriesAt(this.pluginsRoot)).map((id) => [id, pathInside(this.pluginsRoot, id)])
      for (const [pluginDirectoryId, pluginDirectory] of directories) {
