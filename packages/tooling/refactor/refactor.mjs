#!/usr/bin/env node
/**
 * packages/tooling/refactor/refactor.mjs
 * 
 * 基于 TypeScript Compiler API / LanguageService 的项目级 AST 重构与符号索引工具。
 * 支持：文件移动、跨文件符号重命名（F2 同款）、查找引用、全局文本替换。
 * 
 * 用法：
 *   node packages/tooling/refactor/refactor.mjs rename <file> <symbol> <newName> [--line <line>]
 *   node packages/tooling/refactor/refactor.mjs find-refs <file> <symbol> [--line <line>]
 *   node packages/tooling/refactor/refactor.mjs replace-text <search> <replace> [--ext <extensions>]
 */
...
const projectPath = projectIndex < 0 ? null : path.resolve(process.cwd(), process.argv[projectIndex + 1]);
const root = projectPath ? path.dirname(projectPath) : path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../sdk/javascript');

function createLanguageService(configPath = projectPath ?? path.join(root, 'tsconfig.json')) {
...
function handleRename(args) {
  ...
  if (!filePath || !oldName || !newName) {
    console.error(`用法: node packages/tooling/refactor/refactor.mjs rename <file> <oldName> <newName> [--line <N>]`);
    process.exit(1);
  }
...
function handleReplaceText(args) {
  ...
  if (!search || replace === undefined) {
    console.error(`用法: node packages/tooling/refactor/refactor.mjs replace-text <search> <replace> [--ext <ext1,ext2>]`);
    process.exit(1);
  }
...
function handleMove(args) {
  const from = path.resolve(root, args[0]);
  const to = path.resolve(root, args[1]);
  if (!fs.existsSync(from) || fs.existsSync(to)) throw new Error('Invalid move source/destination');
  let workspace = root;
  while (!fs.existsSync(path.join(workspace, '.git'))) {
    const parent = path.dirname(workspace);
    if (parent === workspace) throw new Error('Workspace root not found');
    workspace = parent;
  }
  if (![from, to].every((file) => file.startsWith(workspace + path.sep))) throw new Error('Move outside workspace');
  const original = fs.readFileSync(from, 'utf8');
  const { service } = createLanguageService();
  const edits = service.getEditsForFileRename(from, to, {}, {});
  ...
  const source = ts.createSourceFile(from, original, ts.ScriptTarget.Latest, true);
  const ownEdits = [];
  function visit(node) {
    let literal;
    ...
    else if (ts.isCallExpression(node) && ['import', 'require'].includes(node.expression.getText(source))) literal = node.arguments[0];
    if (literal && ts.isStringLiteralLike(literal) && literal.text.startsWith('.')) {
      let spec = path.relative(path.dirname(to), path.resolve(path.dirname(from), literal.text)).split(path.sep).join('/');
      if (!spec.startsWith('.')) spec = './' + spec;
      ownEdits.push({ start: literal.getStart(source) + 1, end: literal.end - 1, spec });
    }
    ts.forEachChild(node, visit);
  }
  visit(source);
  let rebased = original;
  for (const edit of ownEdits.sort((a, b) => b.start - a.start)) rebased = rebased.slice(0, edit.start) + edit.spec + rebased.slice(edit.end);
  fs.writeFileSync(from, rebased);
  fs.mkdirSync(path.dirname(to), { recursive: true });
  fs.renameSync(from, to);
  console.log(`LanguageService file move: ${edits.length} files updated`);
}
...
命令：
  node packages/tooling/refactor/refactor.mjs move <from> <to> [--project <tsconfig>]
    基于 LanguageService 更新文件引用，用 AST 重定位自身模块路径，再移动文件
  ...
  node packages/tooling/refactor/refactor.mjs replace-text <search> <replace> [--ext .ts,.tsx,.md]
    全仓文本/路径快速批量替换（自动忽略 node_modules/、dist/、.git/）

所有命令可传 --project <tsconfig>，文件路径相对于该配置所在目录；默认使用 JavaScript SDK。
`);
    break;
}
