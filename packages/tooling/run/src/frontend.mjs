...
export async function buildRunFrontends(config) {
  ...
  for (const instance of parsed.frontend.instances) {
    ...
    const manifest = JSON.parse(readFileSync(join(plugin.directory, 'graphvideo.plugin.json'), 'utf8'));
    ...
    const elementEntries = parsed.plugins.frontend.flatMap((entry) => {
      const document = JSON.parse(readFileSync(join(entry.directory, 'graphvideo.plugin.json'), 'utf8'));
    ...
    writeFileSync(html, '<!doctype html><html><head><meta charset="UTF-8"><title>GraphVideo</title></head><body><div id="root"></div><script type="module" src="/entry.tsx"></script></body></html>');
    const alias = {
      '@graphvideo/sdk/protocol': join(repoRoot, 'packages/sdk/javascript/src/protocol/index.ts'),
      ...
      '@graphvideo/workbench': join(repoRoot, 'packages/frontend/workbench/src'),
      '@graphvideo/client': join(repoRoot, 'packages/frontend/client'),
      '@graphvideo/context': join(repoRoot, 'packages/frontend/context'),
      '@graphvideo/ui': join(repoRoot, 'packages/frontend/ui'),
      '@graphvideo/theme': join(repoRoot, 'packages/frontend/theme'),
    };
    await vite.build({ configFile: false, root: output, base: './', cacheDir: join(output, 'cache'), resolve: { alias },
      plugins: [react.default?.() ?? react(), { name: 'run-entry',
        resolveId: (id) => id === '/entry.tsx' ? '\0run-entry' : ['virtual:graphvideo-config', 'virtual:graphvideo-elements'].includes(id) ? `\0${id}` : undefined,
        load: (id) => id === '\0run-entry' ? `import ${JSON.stringify(renderer.replaceAll('\\', '/'))};` : id === '\0virtual:graphvideo-config' ? `export const applicationDefinition=${JSON.stringify(definition)};` : id === '\0virtual:graphvideo-elements' ? `export const elementModules={${elementEntries.join(',')}};` : undefined }],
      build: { outDir: join(output, 'dist'), emptyOutDir: true }, logLevel: 'error' });
    const hostFile = join(output, 'host.mjs');
    const sdkAlias = Object.fromEntries(['protocol', 'node', 'effect', 'plugin', 'analysis', 'agent'].map((name) => [`@graphvideo/sdk/${name}`, join(repoRoot, `packages/sdk/javascript/src/${name}/index.ts`)]));
    sdkAlias['@graphvideo/desktop/command-gate'] = join(desktop, 'host/services/command-gate.mjs');
    ...
    await esbuild.build({ entryPoints: [host], outfile: hostFile, bundle: true, format: 'esm', platform: 'node',
      alias: { ...sdkAlias, '@graphvideo/desktop/electron-window': join(desktop, 'host/effects/electron-window-adapter.mjs'),
        '@graphvideo/desktop/element-catalog': join(desktop, 'host/services/element-catalog.mjs') }, external: ['electron', '*.node', 'typescript'], logLevel: 'silent' });
