...
): { nodes: CausalNode3D[]; edges: CausalEdge3D[] } {
 ...
 */
function pickTierColor(tier: number, maxTier: number, nodeId: string): string {
  if (maxTier === 0) {
    // 只有一层时，通过哈希生成散列色
  ...
  if (ratio === 0) return '#f59e0b' // 起始源：琥珀金
  if (ratio < 0.35) return '#38bdf8' // 转换处理层：电光青
  if (ratio < 0.7) return '#a855f7' // 核心中枢与控制层：深邃幽紫
